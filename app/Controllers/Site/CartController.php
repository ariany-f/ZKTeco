...
class CartController extends Controller{
	...
	public function storeRequest(){
		/**
		 * Enviar email orçamento para os responsaveis pelo setor de compras
		 */
		$request = new Request();
		$data = $request->all();
		...
		$result_mail = Mail::isHtml(true)
			->charset(config('mail.charset'))
			->addFrom(config('mail.to'), config('app.name'))
			->subject('Orçamento Solicitado' . config('app.name'))
			->message(view('mail.request.sale', compact('cart_products', 'amount', 'cliente')))
			->send(config('mail.to'), 'TechScan');
			
			$cart->clear();
			redirect(route('site'), ['success' => 'Seu orçamento foi solicitado com sucesso, em breve entraremos em contato!'], true);

		//Fluxo original de compra
		// Descomentar para funcionar compra normal
